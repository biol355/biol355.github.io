...
# Finding Packages
While I talk about some packages below, here are some neat places to find new or underused packages:
- [R Weekly New Packages](https://rweekly.org/#NewPackages)  
- [AwesomeR's list of packages](https://github.com/qinwf/awesome-R?tab=readme-ov-file#awesome-)  
- [rworks](https://rworks.dev/) regularly has top new package lists. Lots of them!  
...
## worms 
Taxonomy is hard. [worrms](https://docs.ropensci.org/worrms/) makes it easy. Names change. We often want a lot of different information from a taxonomy. One place that is standardized to all marine species is the [World Register of Marine Species](https://www.marinespecies.org/) which I often use - but there are others!.
...
```{r rnoaa, eval= FALSE, cache=TRUE, fig.cap = "Sea Ice at the North Pole from 2000-2010"}
library(rnoaa)
...
The [magrittr](https://magrittr.tidyverse.org/) package is far more than just `|>`. There are a multitude of pipes we don't talk about, different ways of creating repeatable workflows with `. |>` and more. Write a cheat sheet that embraces the full functionality of `magrittr` and maybe even other packages that implement wild pipes!!
\
\
