...
## Shiny Homework
Having a homework for shiny that builds through multiple steps is...kinda nuts. What I'd like you to do is save an R file for each one (prepend 1_, 2_, 3_, etc. so we know which number it is) and then zip it up to submit. For when you post the app, give us a URL!
...
You know how to do this. Use Rstudio or hand-code your own template. Save it as `app.R`. Have it load the data above (straight from the source!) Make sure that it runs. 

## 2. Add a Layout

Choose a layout. It can be a sidebar layout, as we've been doing, or, try setting up something different from [here](https://shiny.rstudio.com/articles/layout-guide.html)
...
Now, add two inputs. One allows users to chose a country. The second allows them to select the type of cases to display (confirmed, death, recovered). Use whatever `*Input` functions you would like. For the country and type, have it show trends and the data table. Make sure your app runs.
...
Using `tmap`, `leafletOutput()`, and `renderLeaflet()` (don't worry, shiny takes care of making it interactive) add a worldmap somewhere (your choice how to do this layout - will be easier with the tabs!) that shows the final # of whatever type you have selected across a worldmap (hey, you know how to merge this data with a worldmap - maybe you have some code for it, that, for additional extra credit, you can source in!) For double your extra credit, when you select a country, have it zoom in on the country (replot but center on that country).

## 10. MOAR MAP Extra Credit (for 2x full credit of another question)
Can you use things at smaller spatial scales than countries?  Or even show just that country and the things in it? And do it well!

## 11. Even More Extra Credit (for full credit of another question)
