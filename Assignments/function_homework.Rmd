...
```{r make_filename, echo = FALSE, eval = FALSE}
make_filename <- function(a_year) {
  str_c("./data/buoydata/44013_", a_year, ".csv", sep = "")
}
```

**Extra credit (2 points):** Check out the [glue package](https://glue.tidyverse.org/) and do the same thing with `glue()`.

```{r make_filename_glue, echo = FALSE, eval = FALSE}
make_filename <- function(a_year) {
  glue::glue("./data/buoydata/44013_{a_year}.csv")
}
```
...
read_buoy <- function(a_year){
  ...
  filename <- make_filename(a_year) #or the str_c from above
  
  a_buoy <- read_csv(filename, na = c("99", "999"))
...
```{r, eval = FALSE, echo = FALSE}
read_buoy <- function(a_year, a_buoy=44013){
  
  filename <- glue::glue("./data/buoydata/{a_buoy}_{a_year}.csv") #or the str_c from above
...
#1987
buoy_2000 <- read.csv("./data/buoydata/44013_1987.csv", na.strings="99")

#2000
buoy_2000 <- read_buoy(2000) %>%
  select(YYYY, MM, DD, WVHT, WTMP) %>%
  rename(Year = YYYY,
         Month = MM,
         Day = DD,
         Wave_Height = WVHT,
...
buoy_2007 <- read_buoy(2007) %>%
  select(X.YY, MM, DD, WVHT, WTMP) %>%
...
# Differences - 2000 has a YYYY column, <1999 is YY and needs to add 1900
# 2013 is all characters and has a X.YY column - starts at 2007
# Everything else is the same
#this is one way - basically, the function below is an extension of what we did in class

make_avg_buoy <- function(a_year, a_buoy = 44013){
  buoy_data <- read_buoy(a_year, a_buoy)
  
  if(a_year < 1999){
    buoy_data <- mutate(buoy_data, YYYY = 1900+YY)
  }
  
  if(a_year > 2006){
    buoy_data <- mutate_all(buoy_data, as.numeric) %>% rename(YYYY = X.YY)
  }
...
```{r, echo = FALSE, eval = FALSE}
## For grading, they should fill all of the comments in. It will either work or not!
## Grade them 3 points per function
```
...
```{r, echo = FALSE, eval = FALSE}
## For grading, they should show meaningful thought here. They might not have the
## data cleaned yet - they might just be starting - be gentle!
```
...
```{r, echo = FALSE, eval = FALSE}
## This is a question open for exploration. They should have fun here!
```