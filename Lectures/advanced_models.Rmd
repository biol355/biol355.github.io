...
```{r neand_boxplot2}
neand_plot_box2 <-   ggplot(neand, 
         aes(x = species, y = lnmass, 
             fill = species)) +
  geom_boxplot()
...
$$lnbrain_{i} \sim \mathcal{N}(\widehat{lnbrain_{i}}, \sigma_2)$$
...
performance::check_predictions(neand_lm)
# neand_dat %>%
...
estimate_relation(neand_lm, 
                  by = c("lnmass", "species")) |> 
  plot(show_data = TRUE,
       point = list(size = 4, alpha = 1),
       ribbon = list(fill = "darkgrey", alpha = 0.5))+
  labs(y = "lnbrain")
```
...
estimate_relation(neand_lm, 
                  by = c("species")) |> 
  plot(show_data = TRUE,
       point = list(position = position_jitter(width = 0.1, height = 0)))+
  labs(y = "lnbrain") +
    labs(subtitle = "Species effects holding\nlnmass at its mean")
