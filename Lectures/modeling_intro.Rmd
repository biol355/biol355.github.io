...
$$Depth_{ij} \sim \mathcal{N}(\hat{depth_{ij}}, \sigma^2)\\
\hat{depth}_{ij} = \alpha_j + \beta * mass_{ij}$$
...
```{r body_plot_1, fig.height = 5}
viz_plot <- ggplot(data = penguins,
       aes(x = body_mass_g, y = bill_depth_mm,
           color = species)) +
  geom_point(aes(y = bill_depth_mm)) +
  stat_smooth(method = "lm", fill = NA, linewidth = 1.5) 
...
```{r body_plot_2, fig.height = 5}

viz_plot +
  geom_vline(xintercept = 4500, color = "red", lty = 2) 
```
...
```{r body_plot_3, fig.height = 5}
mod <- lm(bill_depth_mm ~ body_mass_g + species,
          data = penguins)

pred <- predict(mod, data.frame(body_mass_g = 4500, species = "Gentoo"))

viz_plot +
  geom_vline(xintercept = 4500, color = "red", lty = 2) +
  geom_hline(yintercept = pred, color = "red", lty = 2)
```
...
```{r show_dist, fig.height = 5}
bill_depth_possible <- seq(9, 19, .1)
prob_of_depth <- dnorm(bill_depth_possible, pred, summary(mod)$sigma)
ggplot(data.frame(bd = bill_depth_possible, p = prob_of_depth),
       aes(x = bd, y = p))  +
  geom_line() +
  geom_vline(xintercept = pred, lty = 2, color = "red") +
  annotate(geom = "text", x = 14.7, y = 0.5, label = "estimate", color = "red") +
  labs(y = "Probability Density", x = "Possible Bill Depths")
...
performance::check_predictions(mod) |> plot()
```
...
emmeans(mod, ~species) %>%
  ...
  confint() |>
   as_tibble()|>
  ggplot(aes(y = contrast, x = estimate, xmin = lower.CL, xmax = upper.CL)) +
  geom_pointrange() +
  geom_vline(xintercept = 0, lty = 2) +
  labs(y = "", 
       x = "difference (mm)",
       subtitle = "Difference in Bill Depth at Same Body Mass")
```
--
